...
- **Comprehensive BCP-47 language code normalizer:** Rewrote and extracted `normalizeLanguageCode` from an inline function in the subtitle handler into a shared utility in `src/utils/languages.js`. The old inline version only handled simple `base-XX` regional codes and `pt-BR` — all other BCP-47 subtag types had hyphens stripped and were returned as garbage compacted strings (e.g., `zh-Hant-HK` → `zhhanthk`). The new implementation uses split-based parsing with proper BCP-47 subtag type detection and supports:
  - **Script subtags** (`sr-Cyrl` → `srp`, `mni-Mtei` → `mni`, `zh-Hant` → `zht`): Properly detected by 4-letter length per BCP-47 spec, replacing the old regex that relied on case-sensitivity and couldn't distinguish scripts from regions.
  - **Numeric region codes** (`es-419` → `spn`): UN M.49 numeric regions now work. `es-419` (Latin American Spanish) returns `spn` directly, linking to `spa` via `languageEquivalents`. Other numeric regions (e.g., `es-150`) correctly extract the base language.
  - **Multi-part tags** (`zh-Hant-HK` → `zht`, `sr-Cyrl-ME` → `srp`): Tags with script+region or other compound subtags are handled by extracting the base code and recognizing Chinese script/region variants.
  - **Variant subtags** (`sl-nedis` → `slv`, `de-1996` → `ger`, `ca-valencia` → `cat`): Orthographic, dialectal, and linguistic variant subtags are gracefully handled by extracting the base language code.
  - **Extension subtags** (`en-US-x-twain` → `eng`, `pt-BR-x-aao` → `pob`): Private-use and extension tags are handled by matching earlier subtags first (region/script), then falling back to base extraction.
  - **Chinese variant distinction** (`zh-CN` → `zhs`, `zh-TW` → `zht`): `zh-CN`/`zh-SG`/`zh-Hans` correctly return `zhs` (Simplified), and `zh-TW`/`zh-HK`/`zh-Hant` correctly return `zht` (Traditional), both linked via `languageEquivalents`.
  - Updated `languageCodeSchema` in validation.js to accept 4-letter script subtags (`sr-Cyrl`, `zh-Hant`) and 3-digit numeric regions (`es-419`) — previously rejected with HTTP 400 at the API validation layer before reaching the normalizer.

- **BCP-47 language codes now normalized before sending to subtitle providers:** When users configure regional language variants as target languages (e.g., `es-MX`, `es-AR`, `en-GB`, `zh-CN`), the raw BCP-47 codes are now normalized to ISO-639-2 before being sent to subtitle providers via `searchParams.languages`. Previously, raw BCP-47 codes were passed directly to providers — most providers can't handle BCP-47 and expect ISO-639-2 (3-letter) codes. `toISO6391()` returns `NULL` for all BCP-47 codes, so provider conversion code failed at various fallback paths: OpenSubtitles Auth returned 0 results (total failure), SubSource returned 60 unfiltered garbage results, SubDL and OpenSubtitles V3 returned partial results missing some languages. Added normalization of `searchParams.languages` at both subtitle search entry points (main search and translation selector) using the new shared `normalizeLanguageCode` utility. The raw regional codes (`es-MX`, `es-AR`, etc.) are still preserved for translation button logic — only the provider search queries are normalized (e.g., `[es-MX, es-AR, eng]` → `[spa, eng]` deduplicated). Dedup cache keys also use normalized languages so cache matches correctly regardless of regional variant input.
